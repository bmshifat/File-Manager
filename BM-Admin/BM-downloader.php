<?php
/**
 * BM - veno file manager downloader
 *
 * PHP version >= 5.3
 *
 * @category  PHP
...
if ($json_file && is_numeric($getfile)) {
    ...
    if (file_exists($share_json)) {
        ...
        if (md5($time.$hash) !== $supah ) {
            $_SESSION['error'] = '<i class="fa fa-ban"></i> '.$setUp->getString("access_denied");
...
if ($getzip) {
    ...
    if (!file_exists($file)) {
        $_SESSION['error'] = '<i class="fa fa-times"></i> '.$setUp->getString("nothing_found");
        header('Location:'.$script_url);
        exit;
    }

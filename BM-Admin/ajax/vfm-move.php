<?php
/**
 * BM - veno file manager move or copy files
 *
 * PHP version >= 5.3
 *
 * @category  PHP
...
$setmove = filter_input(INPUT_POST, 'setmove', FILTER_SANITIZE_STRING);
            ...
            if ($downloader->checkFile($pezzo, '../') == true) {
                ...
                if ($copy) {
                    ...
                    if ($actions->checkUserSpace($myfile, $filesize) == false) {
                        Utils::setError('<i class="fa fa-times"></i> '.$setUp->getString('available_space_exhausted').': <strong>'.$basename.'</strong> ('.$setUp->formatSize($filesize).') ');
                        echo "ok";
                        exit;
                    }
                }

<?php
/**
 * BM - veno file manager: ajax/get-remote.php
 *
...
$request = $_POST;
$getdir = filter_var($request['get_location'], FILTER_SANITIZE_STRING);
...
if ($location->editAllowed('../../')
    ...
    include_once '../class/class.actions.php';
    include_once '../class/class.uploader.php';
    include_once '../class/class.logger.php';

    $uploader = new Uploader();

    if (!function_exists('curl_version')) {
        Utils::setError('<span><i class="fa fa-exclamation-triangle"></i> Enable server Curl');
        exit;
    }

    $getfile = filter_input(INPUT_POST, 'get_upload_url', FILTER_VALIDATE_URL);
    $location = filter_input(INPUT_POST, 'get_location', FILTER_SANITIZE_STRING);

    if (!$getfile) {
        Utils::setError('<span><i class="fa fa-exclamation-triangle"></i> '.SetUp::getLangString('invalid_url'));
        exit;
    }
    ...
    if (false === $getcleanfile) {
        $message = ' <span><i class="fa fa-exclamation-triangle"></i> '.SetUp::getLangString('error_downloading_remote_file').'</span>';
        Utils::setError(' <span><i class="fa fa-exclamation-triangle"></i> '.$message.'</span>');
    }

    if (false === file_put_contents($setfile, $getcleanfile)) {
        $message = '<span><i class="fa fa-exclamation-triangle"></i> '.SetUp::getLangString('failed_upload').'</span> ';
        ...
        Utils::setSuccess(' <span><i class="fa fa-check-circle"></i> <strong>'.$filename.'.'.$extension.'</strong></span> ');
    }
    curl_close($ch);
} else {
    $message = '<span><i class="fa fa-exclamation-triangle"></i> '.SetUp::getLangString('upload_not_allowed').'</span> ';
    Utils::setError($message);
}
exit;

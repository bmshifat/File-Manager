<?php
/**
 * BM - veno file manager: ajax/sendpwd.php
 *
...
if (!$dest) {
    print "<div class=\"alert alert-warning\">".$setUp->getString("fill_all_fields")."</div>";
    exit();
}
if (!Utils::checkCaptcha('show_captcha_reset')) {
    print "<div class=\"alert alert-danger\">".$setUp->getString("wrong_captcha")."</div>";
    exit();
}

if (!$updater->findUser($dest, true)) {
    print "<div class=\"alert alert-danger\">".$setUp->getString("email_not_exist")."</div>";
    exit();
}
...
if (!$mail->send()) {
    ...
    print "<div class=\"alert alert-success\">".$setUp->getString('message_sent').": " .$dest."</div>";
}
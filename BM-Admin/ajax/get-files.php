<?php
/**
 * BM - veno file manager: ajax/get-files.php
...
if ($gateKeeper->isAccessAllowed() && $location->editAllowed('../../') && $gateKeeper->isAllowed('view_enable')) {
    ...
    include_once '../fonts/BM-icons.php';
        ...
        foreach ($getfiles as $key => $file) {
            ...
            if ($totcounter > $start) {
                ...
                if ($setUp->getConfig('enable_prettylinks') == true) {
                    ...
                    $downlink = 'BM-Admin/BM-downloader.php?q='.$thislink.'&h='.$dash;
                ...
                if ($file->isValidForAudio()) {
                    ...
                    if ($setUp->getConfig('enable_prettylinks') == true) {
                        ...
                        $linkaudio = "BM-Admin/BM-downloader.php?q=".$thislink."&h=".$dash;
                    }
                    if ($gateKeeper->isAllowed('download_enable')) {
                        $data['icon'] .= '<a type="audio/mp3" id="BM-audio-'.$key.'" class="sm2_button" href="'.$linkaudio.'&audio=play">';
                ...
                if ($gateKeeper->isAllowed('download_enable')) {
                    ...
                    if ($file->isValidForAudio() 
                        ...
                        // $data['icon'] .= '<div class="minibutt" data-toggle="tooltip" title="HERE YOUR MESSAGE"><a class="round-butt" href="'.$downlink.'"';
                        if ($directlinks) {

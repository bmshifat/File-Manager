...
if (!class_exists('Actions', false)) {
    ...
    class Actions
    {
        ...
        public function __construct($location = false)
        {
            ...
            $postnewdir = filter_input(INPUT_POST, 'userdir', FILTER_SANITIZE_STRING);
            $renamedir = filter_input(INPUT_POST, 'newname', FILTER_SANITIZE_STRING);
            ...
            if ($getdel && GateKeeper::isAllowed('delete_enable')) {
                ...
                if ($decode_del) {
                    ...
                    Utils::setError('<i class="fa fa-ban"></i> Character encoding error, remove the file via FTP');
                }
            }
        ...
        public function setDel($getdel)
        {
            ...
            if (Actions::checkDel($getdel) == false) {
                Utils::setError('<i class="fa fa-ban"></i> '.$setUp->getString('not_allowed'));
                return;
            }
            if (is_dir($getdel)) {
                ...
                Utils::setWarning('<span><i class="fa fa-trash-o"></i> <strong>'.substr($getdel, strrpos($getdel, '/') + 1).'</strong></span>');
        ...
        public static function deleteFile($file, $multi = false)
        {
            if (is_file($file)) {
                ...
                if ($multi) {
                    ...
                    Utils::setWarning('<span><i class="fa fa-trash-o"></i> <strong>'.$basepath.'</strong></span> ');
        ...
        public static function renameFile($oldname, $newname, $thenewname, $move = false, $copy = false)
        {
            ...
            if (!file_exists($oldname)) {
                Utils::setError('<span><i class="fa fa-exclamation-circle"></i> <strong>' .$oldname. '</strong> does not exist</span>');
                return false;
            }

            if (Utils::fileExists($newname)) {
                Utils::setWarning('<span><i class="fa fa-info-circle"></i> <strong>' .$thenewname. '</strong> '.$setUp->getString('file_exists').'</span>');
                return false;
            }

            if ($copy) {
                if (!copy($oldname, $newname)) {
                    Utils::setError('<span><i class="fa fa-exclamation-circle"></i> <strong>' .$thenewname. '</strong> can\'t be copied</span>');
                    return false;
                } else {
                ...
                if (!rename($oldname, $newname)) {
                    Utils::setError('<span><i class="fa fa-exclamation-circle"></i> <strong>' .$thenewname. '</strong> can\'t be edited</span>');
                    return false;
                } else {
                    if ($move) {
         ...
         * @param string $relative   relative path '../.' to be called from /BM-Admin/ajax/ instead of '' from index
         *
        ...
        public static function walkDir($dir, $currentdir, $link = false, $relative = '')
        {
            ...
            if (is_dir($relativedir)) {
                ...
                foreach ($scandir as $folder) {
                    ...
                    if ($relativedir.$file == $relative.'./BM-Admin' || substr($file, 0, 1) == '.') {
                        continue;
                    }
                    ...
                    if (is_dir($relativedir.$file)) {
                        ...
                        if ($relativedir.$file."/" == $relative.$currentdir) {
                            $output .= '<i class="fa fa-folder-open"></i> <span class="search-highlight">'.$file.'</span>';
                        } else {
                            if ($link) {
                            ...
                            $output .= '<i class="fa fa-folder-o"></i> '.$file.'</a>';
                        }

                        $scansub = glob($relativedir.$file.'/'.'*', GLOB_ONLYDIR);

                        if (count($scansub) > 0) {
                            $output .= '<span class="toggle-tree"><i class="fa fa-square fa-stack-1x fa-inverse"></i><i class="fa fa-minus-square-o fa-stack-1x tree-toggler"></i></span>';
            ...
            if (GateKeeper::isAllowed('newdir_enable')) {
                if (strlen($dirname) > 0) {
                    ...
                    if (Utils::fileExists($location->getDir(true, false, false, 0).$dirname)) {
                        Utils::setWarning(
                            '<span><i class="fa fa-folder"></i> <strong>'.$dirname.'</strong> '
...
// header('Location:'.$setUp->getConfig('script_url')."?dir=".$location->getDir(false, false, false, 0).$dirname);
         ...
         * @param bool   $multi    called from BM-del.php or BM-move.php
         *

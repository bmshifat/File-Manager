...
if (!class_exists('GateKeeper', false)) {
    ...
    class GateKeeper
    {
         ...
         * @return $message
         */
        public static function init($relative = 'BM-Admin/', $callfrom = '')
        {
            global $gateKeeper;
            global $cookies;
            global $updater;
                ...
                if (Utils::checkCaptcha('show_captcha'.$callfrom)) {
                    ...
                    if (GateKeeper::isUser($postusername, $postuserpass)) {
                        ...
                        if (isset($_SESSION['vfm_user_name_new']) && strlen($_SESSION['vfm_user_name_new']) > 0) {
                            ...
                            $updater->updateUsers('BM-Admin/users/');
                        }
         ...
         * @return image path
         */
        public static function getAvatar($username, $adminarea = 'BM-Admin/', $size = '25')
        {
            $avaimg = md5($username).'.png';
            
            if (!file_exists($adminarea.'_content/avatars/'.$avaimg)) {
                $imgtag = '<img data-name="'.$username.'" class="img-circle avatar avadefault" width="'.$size.'">';
            } else {
                $imgtag = '<img class="img-circle avatar" width="'.$size.'" src="'.SetUp::getConfig('script_url').'BM-Admin/_content/avatars/'.$avaimg.'">';
            }
            return $imgtag;
        }
        ...
        public function setCookie($postusername = false)
        {
            ...
            if (!array_key_exists($postusername, $_REMEMBER)
                ...
                $_REMEMBER[$postusername] = $rmshaved;
                $rmb = '$_REMEMBER = ';
                if (false == (file_put_contents('BM-Admin/users/remember.php', "<?php\n\n $rmb".var_export($_REMEMBER, true).";\n"))) {
         ...
         */
        public static function removeCookie($postusername = false, $path = 'BM-Admin/')
        {
            global $_REMEMBER;
            ...
            if ($postusername && $_REMEMBER) {
                if (array_key_exists($postusername, $_REMEMBER)) {
                    ...
                    $rmb = '$_REMEMBER = ';
                    if (false == (file_put_contents($path.'users/remember.php', "<?php\n\n $rmb".var_export($_REMEMBER, true).";\n"))
                    ) {
